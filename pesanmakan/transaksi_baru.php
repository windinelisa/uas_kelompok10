...
if( empty( $_SESSION['id_user'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		$sql = mysqli_query($koneksi, "INSERT INTO transaksi(no_nota, jenis, nama, alamat, nohp, email, bayar, kembali, total, tanggal, id_user) VALUES('$no_nota', '$jenis', '$nama', '$alamat','$nohp','email', '$bayar', '$kembali', '$total', NOW(), '$id_user')");
...
<form method="post" action="" class="form-horizontal" role="form">
	...
	<div class="form-group">
		...
		<div class="col-sm-3">
			<select name="jenis" class="form-control" id="jenis" required>
				<option value="" disable>--- Pilih Jenis Restoran ---</option>
			<?php

				$q = mysqli_query($koneksi, "SELECT * FROM biaya");
				while($data = mysqli_fetch_array($q)){
					echo '<option value="'.$data['biaya'].'">'.$data['jenis'].' ('.$data['paket'].')</option>';
	...
	<div class="form-group">
		<label for="jenis" class="col-sm-2 control-label">Harga</label>
		<div class="col-sm-3">
